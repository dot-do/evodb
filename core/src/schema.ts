...
import { type Column, type Schema, type SchemaColumn, Type, assertNever } from './types.js';
...
export function serializeSchema(schema: Schema): Uint8Array {
  ...
  view.setUint32(offset, schema.id, true); offset += 4;
  view.setUint32(offset, schema.version, true); offset += 4;
  view.setUint16(offset, schema.columns.length, true); offset += 2;

  for (let i = 0; i < schema.columns.length; i++) {
    const col = schema.columns[i];
    const pathB = pathBytes[i];

    view.setUint16(offset, pathB.length, true); offset += 2;
    result.set(pathB, offset); offset += pathB.length;
    ...
    if (col.defaultValue !== undefined) {
      ...
      view.setUint32(offset, defaultBytes.length, true); offset += 4;
      result.set(defaultBytes, offset); offset += defaultBytes.length;
    }
  }
...
export function deserializeSchema(data: Uint8Array): Schema {
  ...
  for (let i = 0; i < colCount; i++) {
    ...
    const type = data[offset++] as Type;
    const nullable = data[offset++] === 1;
    const hasDefault = data[offset++] === 1;

    let defaultValue: unknown;
    if (hasDefault) {
      const defLen = view.getUint32(offset, true); offset += 4;
...
export function isCompatible(older: Schema, newer: Schema): boolean {
...
export function migrateColumns(columns: Column[], _oldSchema: Schema, newSchema: Schema): Column[] {
...
function promoteColumn(col: Column, toType: Type): Column {
  const values = col.values.map((v, i) => {
    if (col.nulls[i]) return null;
...
function promoteValue(v: unknown, from: Type, to: Type): unknown {
  if (v === null) return null;
  if (to === Type.String) return String(v);
  if (from === Type.Int32 && to === Type.Int64) return BigInt(v as number);
  if (from === Type.Int32 && to === Type.Float64) return v;
  if (from === Type.Int64 && to === Type.Float64) return Number(v);
  return v;
}

/** Estimate serialized size of value */
function estimateValueSize(v: unknown, type: Type): number {
  if (v === null) return 0;
  switch (type) {
    ...
    case Type.String: return 2 + new TextEncoder().encode(v as string).length;
    case Type.Binary: return 4 + (v as Uint8Array).length;
...
function serializeDefault(v: unknown, type: Type): Uint8Array {
  ...
  switch (type) {
    ...
    case Type.Int64: { const b = new Uint8Array(8); new DataView(b.buffer).setBigInt64(0, v as bigint, true); return b; }
...
function deserializeDefault(data: Uint8Array, type: Type): unknown {
  ...
  switch (type) {
    ...
    case Type.String: { const len = view.getUint16(0, true); return decoder.decode(data.subarray(2, 2 + len)); }
    case Type.Binary: { const len = view.getUint32(0, true); return data.slice(4, 4 + len); }
    ...
    case Type.Date: { const len = view.getUint16(0, true); return decoder.decode(data.subarray(2, 2 + len)); }
    default:
...
export interface SchemaDiff {
  added: string[];     // New columns added
  removed: string[];   // Columns removed
  modified: string[];  // Columns with type changes
}

/** Compare two schemas and return the differences */
export function schemaDiff(older: Schema, newer: Schema): SchemaDiff {
  const olderPaths = new Map(older.columns.map(c => [c.path, c]));
