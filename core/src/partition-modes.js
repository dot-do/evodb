...
export const DO_SQLITE_CONFIG = {
    ...
    maxEfficientDataSizeBytes: 10 * 1024 * 1024,
    targetRowsPerPartition: 10000,
    usesR2: false,
    usesDOSqlite: true,
...
export const STANDARD_CONFIG = {
    ...
    minEfficientDataSizeBytes: 1 * 1024 * 1024,
    maxEfficientDataSizeBytes: 10 * 1024 * 1024 * 1024,
    targetRowsPerPartition: 1000000,
    usesR2: true,
    usesDOSqlite: false,
...
export const ENTERPRISE_CONFIG = {
    ...
    minEfficientDataSizeBytes: 100 * 1024 * 1024,
    maxEfficientDataSizeBytes: Number.MAX_SAFE_INTEGER,
    targetRowsPerPartition: 10000000,
    usesR2: true,
    usesDOSqlite: false,
 ...
 */
export const PARTITION_MODE_CONFIGS = {
 ...
 */
export function getPartitionModeConfig(mode) {
 ...
 * ```
 */
export function calculatePartitions(dataSizeBytes, mode, estimatedRowCount) {
 ...
 * ```
 */
export function getPartitionPath(tableLocation, partitionId, mode, extension = 'bin') {
 ...
 */
export function getAllPartitionPaths(tableLocation, partitionCount, mode, extension = 'bin') {
    return Array.from({ length: partitionCount }, (_, i) => getPartitionPath(tableLocation, i, mode, extension));
}
/**
...
export function parsePartitionPath(path) {
    ...
    const mode = dir === 'blobs' ? 'do-sqlite' : 'standard';
    return {
        path,
        tableLocation,
        partitionId,
 ...
 * ```
 */
export function selectPartitionMode(dataSizeBytes, tier = 'pro', options = {}) {
    // Handle forced mode
 ...
 */
export function fitsInSinglePartition(dataSizeBytes, mode) {
    const config = getPartitionModeConfig(mode);
 ...
 */
export function calculatePartitionBoundaries(totalRows, dataSizeBytes, mode) {
 ...
 */
export function estimateDataSize(rowCount, avgRowSizeBytes) {
    return rowCount * avgRowSizeBytes;
}
/**
 * Get mode by maximum partition size
 */
export function getModeByMaxSize(maxBytes) {
    if (maxBytes <= DO_SQLITE_MAX_BYTES)
        return 'do-sqlite';
    if (maxBytes <= STANDARD_MAX_BYTES)
        return 'standard';
    if (maxBytes <= ENTERPRISE_MAX_BYTES)
        return 'enterprise';
    return null;
}
//# sourceMappingURL=partition-modes.js.map