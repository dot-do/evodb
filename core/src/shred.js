...
function inferType(v) {
    ...
    if (typeof v === 'boolean')
        return Type.Bool;
    if (typeof v === 'number')
        return Number.isInteger(v) && Math.abs(v) < 2147483648 ? Type.Int32 : Type.Float64;
    if (typeof v === 'bigint')
        return Type.Int64;
    if (typeof v === 'string') {
...
export function shred(docs, options) {
    const cols = new Map();
    const n = docs.length;
...
function prefixMatchesFilter(prefix, filter) {
    if (!filter)
        return true;
...
function walk(val, path, cols, row, total, filter) {
    ...
    if (type !== Type.Object || val === null) {
        ...
        if (filter && !matchesFilter(path, filter))
            return;
    ...
    for (const key of Object.keys(obj)) {
        ...
        if (filter && !prefixMatchesFilter(childPath, filter))
            continue;
...
function promoteType(a, b) {
    ...
    if ((a === Type.Int32 && b === Type.Float64) || (a === Type.Float64 && b === Type.Int32))
        return Type.Float64;
    // Int32 + Int64 = Int64
    if ((a === Type.Int32 && b === Type.Int64) || (a === Type.Int64 && b === Type.Int32))
        return Type.Int64;
    // Default to string for incompatible types
    return Type.String;
}
/** Reconstruct JSON documents from columns */
export function unshred(columns, rowCount) {
    if (columns.length === 0)
        return [];
    ...
    for (const col of columns) {
        for (let i = 0; i < n; i++) {
            if (!col.nulls[i]) {
                setPath(docs[i], col.path, col.values[i]);
            }
        }
    }
    return docs;
 ...
 */
function ensureContainer(container, key, needArray) {
...
function compressNullBitmap(col) {
    const nulls = col.nulls;
    if (nulls.length < 20)
        return; // Don't compress small arrays
...
export function extractPath(columns, path) {
...
export function extractPaths(columns, paths) {
    const result = {};
...
export function coerceToType(value, toType) {
...
export function appendRows(columns, docs) {
    // Shred new docs
...
export function buildPathIndex(columns) {
