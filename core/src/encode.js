...
function computeStats(col) {
    let min = undefined, max = undefined, nullCount = 0;
    const seen = new Set();
...
function selectAndEncode(col, stats) {
    ...
    if (nonNull.length === 0)
        return { encoding: Encoding.Plain, data: new Uint8Array(0) };
    ...
    if (col.type === Type.String && stats.distinctEst < nonNull.length / 2) {
        return { encoding: Encoding.Dict, data: encodeDictOptimized(nonNull, col.nulls) };
    }
    // Delta for sorted integers - good for timestamps and sequential IDs
    if ((col.type === Type.Int32 || col.type === Type.Int64) && isSorted(nonNull)) {
        return { encoding: Encoding.Delta, data: encodeDeltaTypedArray(nonNull, col.type) };
    }
...
function encodeDictOptimized(values, nulls) {
    const dict = new Map();
...
function encodeDeltaTypedArray(values, type) {
    if (values.length === 0)
...
function encodePlainTypedArray(col) {
...
/** Encode single value */
function encodeValue(v, type, encoder) {
...
/** Decode column data */
export function decode(encoded, rowCount) {
...
function decodeData(data, encoding, type, nulls, rowCount) {
    if (data.length === 0)
...
/** Decode dictionary */
function decodeDict(data, _nulls, rowCount) {
...
/** Decode delta */
function decodeDelta(data, type, nulls, rowCount) {
    if (data.length === 0)
        return new Array(rowCount).fill(null);
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    ...
    for (let i = 0; i < rowCount; i++) {
        ...
        if (valueIdx === 0) {
            ...
            offset += byteSize;
            prev = type === Type.Int64 ? prev + delta : prev + delta;
            values.push(type === Type.Int64 ? prev : Number(prev));
        }
        valueIdx++;
    }
...
/** Decode RLE */
function decodeRLE(data, type, rowCount) {
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    const decoder = new TextDecoder();
    const values = [];
    let offset = 0;
    while (values.length < rowCount && offset < data.length) {
        ...
        offset += 4;
        const isNull = data[offset] === 1;
        offset += 1;
        const value = isNull ? null : decodeValue(data, offset, type, view, decoder);
        offset += isNull ? 0 : valueSize(type, data, offset, view);
        for (let i = 0; i < count && values.length < rowCount; i++)
            values.push(value);
    }
    return values;
}
/** Decode plain */
function decodePlain(data, type, nulls, rowCount) {
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    const decoder = new TextDecoder();
    const values = [];
    let offset = 0;
...
/** Decode single value */
function decodeValue(data, offset, type, view, decoder) {
    switch (type) {
        ...
        case Type.String: {
            const len = view.getUint16(offset, true);
            return decoder.decode(data.subarray(offset + 2, offset + 2 + len));
        }
        case Type.Binary: {
...
function valueSize(type, _data, offset, view) {
    switch (type) {
 ...
 */
export function fastDecodeInt32(data, encoding, nullBitmap, rowCount, options) {
 ...
 * Optimized for snippet 5ms CPU constraint
 */
export function fastDecodeFloat64(data, encoding, nullBitmap, rowCount, options) {
    if (encoding !== Encoding.Plain)
        return null;
    const nonNullCount = countNonNull(nullBitmap, rowCount);
    // Zero-copy path for aligned data
 ...
 */
export function fastDecodeDeltaInt32(data, rowCount) {
 ...
 */
function countNonNull(bitmap, count) {
    let nonNull = 0;
    for (let i = 0; i < count; i++) {
 ...
 */
export function* iterateNonNullIndices(bitmap, count) {
    for (let i = 0; i < count; i++) {
        if ((bitmap[i >>> 3] & (1 << (i & 7))) === 0) {
            yield i;
        }
 ...
 */
export function batchDecode(encoded, rowCount, options) {
