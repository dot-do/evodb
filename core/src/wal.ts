...
export function createWalEntry(doc: unknown, lsn: bigint, op: WalOp = WalOp.Insert): WalEntry {
  ...
  for (const col of encoded) {
    ...
    view.setUint16(off, pathBytes.length, true); off += 2;
    header.set(pathBytes, off); off += pathBytes.length;
    header[off++] = col.type;
    header[off++] = col.encoding;
    view.setUint32(off, col.nullBitmap.length, true); off += 4;
    header.set(col.nullBitmap, off); off += col.nullBitmap.length;
    view.setUint32(off, col.data.length, true); off += 4;
...
export function serializeWalEntry(entry: WalEntry): Uint8Array {
  ...
  view.setBigUint64(offset, entry.lsn, true); offset += 8;
  view.setBigUint64(offset, entry.timestamp, true); offset += 8;
...
export function deserializeWalEntry(data: Uint8Array): WalEntry {
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  let offset = 0;

  const lsn = view.getBigUint64(offset, true); offset += 8;
  ...
  // Verify checksum
  if (crc32(entryData) !== checksum) throw new Error('WAL entry checksum mismatch');

  return { lsn, timestamp, op, flags, data: entryData, checksum };
}

/** Batch multiple WAL entries */
export function batchWalEntries(entries: WalEntry[]): Uint8Array {
  ...
  for (const s of serialized) {
    ...
    result.set(s, offset); offset += s.length;
  }
...
/** Unbatch WAL entries */
export function unbatchWalEntries(data: Uint8Array): WalEntry[] {
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  let offset = 0;
...
export function getWalRange(data: Uint8Array): { minLsn: bigint; maxLsn: bigint; count: number } {
  const entries = unbatchWalEntries(data);
  if (entries.length === 0) return { minLsn: 0n, maxLsn: 0n, count: 0 };

  let minLsn = entries[0].lsn;
  let maxLsn = entries[0].lsn;
  for (const e of entries) {
    if (e.lsn < minLsn) minLsn = e.lsn;
    if (e.lsn > maxLsn) maxLsn = e.lsn;
  }
